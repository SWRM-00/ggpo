...
Poll::Poll(void) : _handle_count(0), _start_time(0)
{
	/*
...
bool Poll::Pump(int timeout)
{
	...
	if (res >= WAIT_OBJECT_0 && res < WAIT_OBJECT_0 + _handle_count) {
		i = res - WAIT_OBJECT_0;
		finished =
		    !_handle_sinks[i].sink->OnHandlePoll(_handle_sinks[i].cookie) ||
		    finished;
	}
	...
	for (i = 0; i < _periodic_sinks.size(); i++) {
		...
		if (cb.interval + cb.last_fired <= elapsed) {
			...
			finished =
			    !cb.sink->OnPeriodicPoll(cb.cookie, cb.last_fired) || finished;
		}
	}
