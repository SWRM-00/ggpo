...
Sync::Sync(NetworkMsg::connect_status *connect_status)
    : _local_connect_status(connect_status), _input_queues(NULL)
{
	_framecount = 0;
...
Sync::~Sync()
{
	...
	for (int i = 0; i < ARRAY_SIZE(_savedstate.frames); i++) {
		_callbacks.free_buffer(_savedstate.frames[i].buf, _user_data);
	}
...
bool Sync::AddLocalInput(int queue, GameInput &input)
{
	...
	if (_framecount >= _max_prediction_frames &&
	    frames_behind >= _max_prediction_frames) {
...
int Sync::GetConfirmedInputs(void *values, int size, int frame)
{
	...
	for (int i = 0; i < _config.num_players; i++) {
		...
		memcpy(
		    output + (i * _config.input_size), input.bits, _config.input_size);
	}
...
void Sync::LoadFrame(int frame)
{
	...
	_callbacks.load_game_state(state->buf, state->cbuf, _user_data);
...
void Sync::SaveCurrentFrame()
{
	...
	SavedFrame *state = _savedstate.frames + _savedstate.head;
	if (state->buf) {
		_callbacks.free_buffer(state->buf, _user_data);
		state->buf = NULL;
	}
	state->frame = _framecount;
	_callbacks.save_game_state(
	    &state->buf, &state->cbuf, &state->checksum, state->frame, _user_data);
