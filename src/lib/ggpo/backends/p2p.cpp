...
Peer2PeerBackend::Peer2PeerBackend(GGPOSessionCallbacks *cb,
    ...
    : _num_players(num_players), _input_size(input_size),
      ...
      _num_spectators(0), _next_spectator_frame(0), _user_data(user)
{
	_callbacks = *cb;
	_synchronizing = true;
	...
	_endpoints = new NetworkProtocol[_num_players];
...
void Peer2PeerBackend::AddRemotePlayer(GGPOConnectionPlayerID player, int queue)
{
	/*
	...
	_endpoints[queue].Init(
	    &_network, _poll, queue, player, _local_connect_status);
	_endpoints[queue].SetDisconnectTimeout(_disconnect_timeout);
	_endpoints[queue].SetDisconnectNotifyStart(_disconnect_notify_start);
	_endpoints[queue].Synchronize();
}

GGPOErrorCode Peer2PeerBackend::AddSpectator(GGPOConnectionPlayerID connection)
{
	...
	_spectators[queue].Init(
	    &_network, _poll, queue + 1000, connection, _local_connect_status);
	...
	if (!_sync.InRollback()) {
		...
		if (!_synchronizing) {
			...
			if (total_min_confirmed >= 0) {
				...
				if (_num_spectators > 0) {
					while (_next_spectator_frame <= total_min_confirmed) {
						...
						_sync.GetConfirmedInputs(input.bits,
						    _input_size * _num_players,
						    _next_spectator_frame);
			...
			if (current_frame > _next_recommended_sleep) {
				...
				for (int i = 0; i < _num_players; i++) {
					interval =
					    MAX(interval, _endpoints[i].RecommendFrameDelay());
				}
...
int Peer2PeerBackend::Poll2Players(int current_frame)
{
	...
	for (i = 0; i < _num_players; i++) {
		...
		Log("  local endp: connected = %d, last_received = %d, "
		    "total_min_confirmed = %d.\n",
		    !_local_connect_status[i].disconnected,
		    _local_connect_status[i].last_frame,
		    total_min_confirmed);
...
int Peer2PeerBackend::PollNPlayers(int current_frame)
{
	...
	for (queue = 0; queue < _num_players; queue++) {
		...
		for (i = 0; i < _num_players; i++) {
			// we're going to do a lot of logic here in consideration of
			// endpoint i. keep accumulating the minimum confirmed point for all
			// n*n packets and throw away the rest.
			if (_endpoints[i].IsRunning()) {
				bool connected =
				    _endpoints[i].GetPeerConnectStatus(queue, &last_received);

				queue_connected = queue_connected && connected;
				queue_min_confirmed = MIN(last_received, queue_min_confirmed);
				Log("  endpoint %d: connected = %d, last_received = %d, "
				    "queue_min_confirmed = %d.\n",
				    i,
				    connected,
				    last_received,
				    queue_min_confirmed);
		...
		if (!_local_connect_status[queue].disconnected) {
			queue_min_confirmed = MIN(
			    _local_connect_status[queue].last_frame, queue_min_confirmed);
		}
		Log("  local endp: connected = %d, last_received = %d, "
		    "queue_min_confirmed = %d.\n",
		    !_local_connect_status[queue].disconnected,
		    _local_connect_status[queue].last_frame,
		    queue_min_confirmed);
		...
		if (queue_connected) {
			...
			// check to see if this disconnect notification is further back than
			// we've been before.  If so, we need to re-adjust.  This can happen
			// when we detect our own disconnect at frame n and later receive a
			// disconnect notification for frame n-1.
			if (!_local_connect_status[queue].disconnected ||
			    _local_connect_status[queue].last_frame > queue_min_confirmed) {
...
GGPOErrorCode Peer2PeerBackend::AddPlayer(
    GGPOPlayer *player, GGPOPlayerHandle *handle)
{
	if (player->type == GGPO_PLAYERTYPE_SPECTATOR) {
		return AddSpectator(player->u.remote.player_id);
	}
	...
	*handle = QueueToPlayerHandle(queue);
	...
	if (player->type == GGPO_PLAYERTYPE_REMOTE) {
		AddRemotePlayer(player->u.remote.player_id, queue);
	}
	return GGPO_OK;
}

GGPOErrorCode Peer2PeerBackend::AddLocalInput(
    GGPOPlayerHandle player, void *values, int size)
{
	...
	if (input.frame !=
	    GameInput::NullFrame) { // xxx: <- comment why this is the case
		// Update the local connect status state to indicate that we've got a
		// confirmed local frame for this player.  this must come first so it
		// gets incorporated into the next packet we send.

		Log("setting local connect status for local queue %d to %d",
		    queue,
		    input.frame);
...
GGPOErrorCode Peer2PeerBackend::SyncInput(
...
void Peer2PeerBackend::OnUdpProtocolSpectatorEvent(
    NetworkProtocol::Event &evt, int queue)
{
...
void Peer2PeerBackend::OnUdpProtocolEvent(
    NetworkProtocol::Event &evt, GGPOPlayerHandle handle)
{
	GGPOEvent info;
...
GGPOErrorCode Peer2PeerBackend::DisconnectPlayer(GGPOPlayerHandle player)
{
	...
	if (!_endpoints[queue].IsInitialized()) {
		int current_frame = _sync.GetFrameCount();
		// xxx: we should be tracking who the local player is, but for now
		// assume that if the endpoint is not initalized, this must be the local
		// player.
...
void Peer2PeerBackend::DisconnectPlayerQueue(int queue, int syncto)
{
	...
	Log("Changing queue %d local connect status for last frame from %d to %d "
	    "on disconnect request (current: %d).\n",
	    queue,
	    _local_connect_status[queue].last_frame,
	    syncto,
	    framecount);
	...
	if (syncto < framecount) {
		Log("adjusting simulation to account for the fact that %d disconnected "
		    "@ %d.\n",
		    queue,
		    syncto);
...
GGPOErrorCode Peer2PeerBackend::GetNetworkStats(
    GGPONetworkStats *stats, GGPOPlayerHandle player)
{
	int queue;
	GGPOErrorCode result;
...
GGPOErrorCode Peer2PeerBackend::SetFrameDelay(
    GGPOPlayerHandle player, int delay)
{
	int queue;
	GGPOErrorCode result;
...
GGPOErrorCode Peer2PeerBackend::PlayerHandleToQueue(
    GGPOPlayerHandle player, int *queue)
{
...
void Peer2PeerBackend::CheckInitialSync()
{
	...
	if (_synchronizing) {
		...
		for (i = 0; i < _num_players; i++) {
			// xxx: IsInitialized() must go... we're actually using it as a
			// proxy for "represents the local player"
			if (_endpoints[i].IsInitialized() &&
