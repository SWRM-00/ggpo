...
namespace neosmart {
	...
	// which tracks the progress of the caller's multi-object wait and
	// dispatches responses accordingly. One neosmart_wfmo_t struct is shared
	// for all events in a single WFMO call
	...
	// A neosmart_wfmo_info_t object is registered with each event waited on in
	// a WFMO This reference to neosmart_wfmo_t_ is how the event knows whom to
	...
	int UnlockedWaitForEvent(neosmart_event_t event, uint64_t milliseconds)
	{
		...
		if (!event->State) {
			...
			do {
				// Regardless of whether it's an auto-reset or manual-reset
				// event: wait to obtain the event, then lock anyone else out
				if (milliseconds != (uint64_t)-1) {
					result = pthread_cond_timedwait(
					    &event->CVariable, &event->Mutex, &ts);
				}
				else {
					result =
					    pthread_cond_wait(&event->CVariable, &event->Mutex);
				}
		...
		// Else we're trying to obtain a manual reset event with a signaled
	...
	#ifdef WFMO
	int WaitForMultipleEvents(neosmart_event_t *events,
	    int count,
	    bool waitAll,
	    uint64_t milliseconds)
	{
		...
		return WaitForMultipleEvents(
		    events, count, waitAll, milliseconds, unused);
	}
	    ...
	    int &waitIndex)
	{
		...
		for (int i = 0; i < count; ++i) {
			...
			// Before adding this wait to the list of registered waits, let's
			// clean up old, expired waits while we have the event lock anyway
		...
		// We set the `done` flag above in case of WaitAny and at least one
		// event was set. But we need to check again here if we were doing a
		// WaitAll or else we'll incorrectly return WAIT_TIMEOUT.
		...
		while (!done) {
			...
			// If we're waiting for all events, assume we're done and check if
			// there's an event that hasn't fired But if we're waiting for just
			// one event, assume we're not done until we find a fired event
		...
		if (event->AutoReset) {
			...
			while (!event->RegisteredWaits.empty()) {
				...
				if (i->Waiter->WaitAll) {
					...
					// != 0 but the only time it'll be equal to zero is if we're
					// the last event, so no one else will be checking the
					// StillWaiting flag. We're good to go without it.
				}
				else {
	...
	int PulseEvent(neosmart_event_t event)
	{
		...
		// intention of reducing code duplication, but in reality this is what
		// any PulseEvent() implementation must look like. The only overhead
		// (function calls aside, which your compiler will likely optimize away,
		// anyway), is if only WFMO auto-reset waits are active there will be
		// overhead to unnecessarily obtain the event mutex for ResetEvent()
		// after. In all other cases (being no pending waits, WFMO manual-reset
		// waits, or any WFSO waits), the event mutex must first be released for
		// the waiting thread to resume action prior to locking the mutex again
		// in order to set the event state to unsignaled, or else the waiting
		// threads will loop back into a wait (due to checks for spurious
		// CVariable wakeups).
...
namespace neosmart {
	...
	int WaitForEvent(neosmart_event_t event, uint64_t milliseconds)
	{
		...
		// WaitForSingleObject(Ex) and WaitForMultipleObjects(Ex) only support
		// 32-bit timeout
		if (milliseconds == ((uint64_t)-1) || (milliseconds >> 32) == 0) {
			result = WaitForSingleObject(
			    handle, static_cast<uint32_t>(milliseconds));
		}
		else {
		...
		if (result == WAIT_OBJECT_0 || result == WAIT_ABANDONED) {
			// We must swallow WAIT_ABANDONED because there is no such
			// equivalent on *nix
			return 0;
		}
	    ...
	    uint64_t milliseconds)
	{
		int index = 0;
		return WaitForMultipleEvents(
		    events, count, waitAll, milliseconds, index);
	}
	    ...
	    int &index)
	{
		...
		if (milliseconds == ((uint64_t)-1) || (milliseconds >> 32) == 0) {
			result = WaitForMultipleObjects(
			    count, handles, waitAll, static_cast<uint32_t>(milliseconds));
		}
		else {
			// Cannot wait for 0xFFFFFFFF because that means infinity to WIN32
			uint32_t waitUnit = (INFINITE - 1);
			...
			uint32_t result =
			    WaitForMultipleObjects(count, handles, waitAll, remainder);
			while (result == WAIT_TIMEOUT && rounds-- != 0) {
				result =
				    WaitForMultipleObjects(count, handles, waitAll, waitUnit);
			}
		}
