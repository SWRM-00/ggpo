...
void InputQueue::DiscardConfirmedFrames(int frame)
{
	...
	Log("discarding confirmed frames up to %d (last_added:%d length:%d "
	    "[head:%d tail:%d]).\n",
	    frame,
	    _last_added_frame,
	    _length,
...
{
	ASSERT(_first_incorrect_frame == GameInput::NullFrame ||
	       requested_frame < _first_incorrect_frame);
...
bool InputQueue::GetInput(int requested_frame, GameInput *input)
{
	...
	if (_prediction.frame == GameInput::NullFrame) {
		...
		if (requested_frame == 0) {
			Log("basing new prediction frame from nothing, you're client wants "
			    "frame 0.\n");
			_prediction.erase();
		}
		else if (_last_added_frame == GameInput::NullFrame) {
			Log("basing new prediction frame from nothing, since we have no "
			    "frames yet.\n");
			_prediction.erase();
		}
		else {
			Log("basing new prediction frame from previously added frame "
			    "(queue entry:%d, frame:%d).\n",
			    PREVIOUS_FRAME(_head),
			    _inputs[PREVIOUS_FRAME(_head)].frame);
...
void InputQueue::AddDelayedInputToQueue(GameInput &input, int frame_number)
{
	...
	ASSERT(_last_added_frame == GameInput::NullFrame ||
	       frame_number == _last_added_frame + 1);

	ASSERT(frame_number == 0 ||
	       _inputs[PREVIOUS_FRAME(_head)].frame == frame_number - 1);
	...
	if (_prediction.frame != GameInput::NullFrame) {
		 ...
		 */
		if (_first_incorrect_frame == GameInput::NullFrame &&
		    !_prediction.equal(input, true)) {
		 ...
		 * count up.
		 */
		if (_prediction.frame == _last_frame_requested &&
		    _first_incorrect_frame == GameInput::NullFrame) {
...
int InputQueue::AdvanceQueueHead(int frame)
{
	...
	int expected_frame =
	    _first_frame ? 0 : _inputs[PREVIOUS_FRAME(_head)].frame + 1;
